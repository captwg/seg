    ...
    def memory_efficient_forward(self, outputs, targets, cost=["cls", "box", "mask"]):
        ...
        for b in range(bs):
            ...
            with autocast(device_type, enabled=False):
                # Matching is numerically sensitive and is not part of the backward
                # graph. Keep every cost component in FP32 even when training uses AMP.
                out_bbox = outputs["pred_boxes"][b].float()
                tgt_bbox = targets[b]["boxes"].to(device=out_bbox.device, dtype=torch.float32)
                if 'box' in cost:
                    ...
                    cost_bbox = out_bbox.new_zeros(())
                    cost_giou = out_bbox.new_zeros(())

                out_logits = outputs["pred_logits"][b].float()
                ...
                if 'mask' in cost:
                    out_mask = outputs["pred_masks"][b].float()[:, None]
                    # gt masks are already padded when preparing target
                    tgt_mask = targets[b]["masks"].to(
                        device=out_mask.device, dtype=torch.float32
                    )[:, None]
                    # all masks share the same set of points for efficient matching!
                    point_coords = torch.rand(
                        1, self.num_points, 2, device=out_mask.device, dtype=torch.float32
                    )
                    # get gt labels
                    ...
                    cost_mask = out_bbox.new_zeros(())
                    cost_dice = out_bbox.new_zeros(())
                ...
                if invalid:
                    ...
                    for name in invalid:
                        ...
                        finite_range = (
                            ...
                            if finite.numel() else "empty"
                        )
                        summaries.append(
                            f"{name}(shape={tuple(value.shape)}, dtype={value.dtype}, "
                            f"nonfinite={(~torch.isfinite(value)).sum().item()}, "
                            f"finite_range={finite_range})"
                        )
                    image_id = targets[b].get("image_id", "unknown")
                    raise FloatingPointError(
                        "HungarianMatcher produced non-finite values: "
                        f"batch_index={b}, image_id={image_id}, invalid={invalid}; "
                        + "; ".join(summaries)
                    )
